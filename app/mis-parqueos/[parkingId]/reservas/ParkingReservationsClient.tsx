...
type ParkingReservationsClientProps = {
  parking: Parking;
  reservations: DemoOwnerReservation[];
};
...
export default function ParkingReservationsClient({
  ...
  const filteredReservations = useMemo(() => {
    ...
    return reservations.filter((reservation) => {
      ...
      const matchesQuery = normalizedQuery
        ? normalizeText(
            `${reservation.customer.name} ${reservation.customer.email} ${reservation.vehiclePlate} ${reservation.vehicleCategory}`,
  ...
  const totalRevenue = reservations.reduce((total, reservation) => total + reservation.amountPaid, 0);
  const totalReservedSpaces = reservations.reduce(
    (total, reservation) => total + reservation.spacesReserved,
    0,
  );
    ...
    <section className={styles.page}>
      <div className={styles.shell}>
        ...
        <section className={styles.summaryGrid}>
          ...
          <article className={styles.summaryCard}>
            <strong>{totalReservedSpaces}</strong>
            <span>espacios comprometidos</span>
          </article>
          <article className={styles.summaryCard}>
        ...
        <section className={styles.banner}>
          <div>
            ...
            <p>
              Confirmadas: {reservedCount} | Usadas: {usedCount} | Canceladas: {cancelledCount}
            </p>
          </div>
          <Link href="/publicar-parqueo" className={styles.bannerAction}>
            Ajustar publicacion
          </Link>
        </section>
              ...
              <article key={reservation.id} className={styles.card}>
                ...
                <div className={styles.infoGrid}>
                  ...
                  <div>
                    ...
                    <strong>
                      {reservation.vehicleCategory} | {reservation.vehiclePlate}
                    </strong>
                  </div>
                  <div>
                    <span className={styles.label}>Espacios</span>
                    <strong>{reservation.spacesReserved}</strong>
                  </div>
                  <div>
          ...
          <div className={styles.modalCard}>
            ...
            <div className={styles.modalGrid}>
              <div className={styles.modalPanel}>
                ...
                <p>{activeReservation.customer.phone}</p>
              </div>
              <div className={styles.modalPanel}>
              ...
              <div className={styles.modalPanel}>
                ...
                <p>Placa: {activeReservation.vehiclePlate}</p>
              </div>
              <div className={styles.modalPanel}>
                <span className={styles.label}>Monto y pago</span>
                <p>{formatCurrency(activeReservation.amountPaid)}</p>
                <p>{activeReservation.paymentMethod}</p>
              </div>
              <div className={styles.modalPanel}>
                <span className={styles.label}>Operacion</span>
                <p>{activeReservation.spacesReserved} espacio(s)</p>
                <p>Canal: {activeReservation.source}</p>
              </div>
              <div className={`${styles.modalPanel} ${styles.modalPanelWide}`}>
                <span className={styles.label}>Notas operativas</span>
