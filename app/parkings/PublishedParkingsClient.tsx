...
import type { SavedParkingState } from "@/app/saved/_lib/saved-helpers";
import Image from "next/image";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import styles from "./published-parkings.module.css";
import {
...
type PublishedParkingsClientProps = {
  ...
  initialSavedState: SavedParkingState;
...
export default function PublishedParkingsClient({
  ...
  const departments = getDepartments(parkings);
  const municipalityOptions = getMunicipalitiesByDepartment(parkings, formFilters.department);
  const activeFilterCount = countActiveFilters(appliedFilters);
  const filteredParkings = filterParkings(parkings, appliedFilters);
  ...
  function applyFilters(nextFilters: ParkingFilters) {
    const normalizedFilters = normalizeFilters(parkings, nextFilters);
  ...
  function openSaveModal(parking: Parking) {
    setSaveTarget(parking);
  }
  ...
  async function removeFavorite(parking: Parking) {
    const exists = savedState.favoriteIds.includes(parking.id);
    ...
    if (!response.ok || !payload.state) {
      setNotification(payload.error ?? "No se pudo actualizar este favorito.");
      return;
    }

    setSavedState(payload.state);
    setNotification(`Parqueo eliminado de favoritos: ${parking.name}`);
  }

  async function saveFavorite(folderId: string | null) {
    if (!saveTarget) {
      return;
    }
    ...
    setNotification(
      folderId === null
        ? `Parqueo guardado en favoritos: ${saveTarget.name}`
        : `Parqueo guardado en ${payload.state.folders.find((folder) => folder.id === folderId)?.name ?? "una carpeta"}`,
    );
    closeSaveModal();
  }

  async function toggleFavorite(parking: Parking) {
    if (!isAuthenticated) {
      setNotification("Inicia sesion para guardar parqueos en tu biblioteca.");
      return;
    }

    if (savedState.favoriteIds.includes(parking.id)) {
      await removeFavorite(parking);
      return;
    }

    openSaveModal(parking);
  }
    ...
    <section className={styles.page}>
          ...
          <div className={styles.resultsHeader}>
            <div>
              ...
              <p>
                ...
                {parkings.length} parqueos publicados.
              </p>
            </div>
          </div>
          ...
          {filteredParkings.length > 0 ? (
            <div className={styles.cardGrid}>
              {filteredParkings.map((parking) => {
                ...
                const isFavorite = savedState.favoriteIds.includes(parking.id);
      ...
      {saveTarget ? (
        <FolderAssignmentModal
          ...
          onConfirm={(folderId) => void saveFavorite(folderId)}
          parking={saveTarget}
          title="Guardar parqueo"
        />
      ) : null}
    </section>
  );
...
function getDepartments(parkings: Parking[]) {
  return Array.from(new Set(parkings.map((parking) => parking.department))).sort((left, right) =>
...
function getMunicipalitiesByDepartment(parkings: Parking[], department: string) {
  const source = department
    ? parkings.filter((parking) => parking.department === department)
    : parkings;
...
function normalizeFilters(parkings: Parking[], filters: ParkingFilters): ParkingFilters {
  const nextDepartment = filters.department.trim();
  const nextMunicipality = nextDepartment ? filters.municipality.trim() : "";
  const availableMunicipalities = getMunicipalitiesByDepartment(parkings, nextDepartment);
...
function filterParkings(parkings: Parking[], filters: ParkingFilters) {
...
function getToday(): ParkingDayKey {
  const days: ParkingDayKey[] = [
    "domingo",
    "lunes",
    "martes",
    "miercoles",
