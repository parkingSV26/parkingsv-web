...
export default function SettingsClient({ userId }: SettingsClientProps) {
  const [savedPreferences, setSavedPreferences] = useState<ParkingPreferences>(() =>
    readStoredPreferences(userId),
  );
  const [draftPreferences, setDraftPreferences] = useState<ParkingPreferences>(() =>
    readStoredPreferences(userId),
  );
  ...
  function handleSave() {
    const didPersist = persistPreferences(draftPreferences, userId);
